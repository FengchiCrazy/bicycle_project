...
`vim`被称为“编辑器之神”。它的学习曲线很陡峭，全部操作都可以在大键盘上实现，不需要使用鼠标和小键盘，进而提升效率。对于习惯了在windows下的文本编辑器的同学而言，可能刚开始会很不适应。但是当最开始的难关过去之后，会发现这个编辑器没那么难用了:joy:。问题在于很多追求稳定而没有装图形界面的服务器上，`vim`似乎是唯一的选择，而许多公司就是喜欢这种非常稳定不需要图形界面的服务器。所以学好`vim`对工作还是有点帮助的。
...
| 6 | `select * from sxj limit 10;` | 查询语句，可以自行定义，这一步可以理解为测试，为后面导出文件做准备 |
| 7 | `exit;` | 退出Hive，进入Linux命令行，进行文件导出操作 |
| 8 | `hive -e"use bicycle; select * from sxj limit 10;" > result.csv` | 注 |

注：最后这条语句有点复杂，但是是把hive结果导出文件的核心操作。分解下来看是这样：

1. `hive`是这条语句的核心，标明这条命令是由Hive执行的
2. `-e "..."` 在Linux的世界中，命令之后的`-`都可以理解为该命令的`选项`。几乎每一个Linux的命令都有选项，之前的表格中的Linux也都有，有兴趣的同学可以自行搜索。在这条语句中 `-e`的意思是其后双引号`""`中的语句为交给hive执行的查询语句。所以我们这里输入了刚才在测试时候输入的`use bicycle; select * from sxj limit 10;`(注意不要忘记`;`)。所以在上文中提到查询语句可以理解为后面导出文件做测试。hive中还有很多选项，可以通过`hive -H`查看，然后自己试试看是干什么用的。
...
6. 至于接收文件的文件名，我们这里选择了`result.csv`，其实并没有太大关系，自己可以随意起一个好记的名字，比如`result_fengchi`也可以，它们都是保存结果的文本文件。与Windows不同，Linux中文件并没有特别强的扩展名的概念，相同扩展名的文件只是为了提醒他们是同一个类型的。比如`csv`就代表他们是有分隔符的表格形式。
