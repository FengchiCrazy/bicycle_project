...
*tips:*第一次登陆个人账号时，建议修改密码，命令行下输入`passwd`，按照英文提示进行操作即可。在Linux系统下输入的密码是不可见的，所以自己注意不要输错了哦~
...
- 目前在Hive里只有`tran`交易流水表和`sxj`上下架表。其他的两个数据表，包括交易流水表和上下架表的原始数据，均存放在服务器`/home/dmc/project/bike`目录下。其中交易流水表是`location_list.xls`中的`sheet1`中，需要注意的是并不是每一个有交易记录的站点都能找到对应的经纬度。日级别天气表是`weather.csv`。详细的字段说明在`data_dictionary.docx`中，请下载查看。
